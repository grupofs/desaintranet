...
$(function() {
	 ...
	 * @returns {Object}
	 */
	s2TipoProducto.init = function (el) {
		// En caso no se envie el elemento, se toma uno por defecto
		el = (typeof el === 'undefined') ? $(s2TipoProducto.elDefault) : el;

		this.select2 = el.select2({
			ajax: {
				url: BASE_URL + 'ar/ope/cttramite/autocompletado_tipo_producto',
				method: 'POST',
				dataType: 'json',
				delay: 250,
			...
			templateResult: function (res) {
				...
				s2TipoProducto.selected.id = 0;
				s2TipoProducto.validate = 0;
				return res.text;
			},
			templateSelection: function (res) {
				if (res.id !== "") {
					// Solo se ejecuta cuando se realiza una busqueda
					if (s2TipoProducto.selected.id == 0) {
						// Solo cuando aun no es selccionado se marca.
						if (s2TipoProducto.validate == 0) {
							if (typeof __elegirTipoProducto == "function") {
								__elegirTipoProducto(res);
							}
						}
					}
					// Se cambia el estado para no voler a seleccionar
					s2TipoProducto.validate = 1;
					return res.text;
				} else {
					return 'Elegir';
				}
