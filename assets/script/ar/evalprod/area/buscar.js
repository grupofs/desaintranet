...
$(function() {
     ...
     * @returns {Object}
     */
    s2Area.init = function (el) {
        // En caso no se envie el elemento, se toma uno por defecto
        el = (typeof el === 'undefined') ? $(s2Area.elDefault) : el;

        this.select2 = el.select2({
            ajax: {
                url: BASE_URL + 'ar/evalprod/carea/autocompletado',
                method: 'POST',
                dataType: 'json',
                delay: 250,
            ...
            templateSelection: function (res) {
                if (res.id !== "") {
                    ...
                    if (s2Area.selected.id == 0) {
                        // Solo cuando aun no es selccionado se marca.
                        if (s2Area.validate == 0) {
                            if (typeof __elegirArea == "function") {
                                __elegirArea(res);
                            }
                        }
                    }
