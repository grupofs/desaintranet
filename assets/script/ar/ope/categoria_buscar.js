...
$(function() {
	 ...
	 * @returns {Object}
	 */
	s2Categoria.init = function (el) {
		// En caso no se envie el elemento, se toma uno por defecto
		el = (typeof el === 'undefined') ? $(s2Categoria.elDefault) : el;

		this.select2 = el.select2({
			ajax: {
				url: BASE_URL + 'ar/ope/ccategoriaxcliente/autocompletado',
				method: 'POST',
				dataType: 'json',
				delay: 250,
			...
			templateResult: function (res) {
				...
				s2Categoria.selected.id = 0;
				s2Categoria.validate = 0;
				return res.text;
			},
			templateSelection: function (res) {
				if (res.id !== "") {
					// Solo se ejecuta cuando se realiza una busqueda
					if (s2Categoria.selected.id == 0) {
						// Solo cuando aun no es selccionado se marca.
						if (s2Categoria.validate == 0) {
							if (typeof __elegirCategoriaXCliente == "function") {
								__elegirCategoriaXCliente(res);
							}
						}
					}
					// Se cambia el estado para no voler a seleccionar
					s2Categoria.validate = 1;
					return res.text;
				} else {
					return 'Elegir';
				}
