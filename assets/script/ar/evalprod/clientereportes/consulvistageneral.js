...
getListvistageneral = function (param) {
	...
	oTable_vistageneral = $('#tblvistageneral').DataTable({
		...
		'columns': [
			...
			{data: 'observacion_cli', orderable: false, targets: 40},
		]
	});
	/***************************************************/
	// Enumeracion
...
$('#btnReporteExel').click(function() {
	const button = $(this);
	$.ajax({
		url: baseurl + 'ar/tramites/cexcelExport/exportar_registros_evalprod',
		method: 'POST',
		data: getParams(),
		dataType: 'json',
		beforeSend: function() {
