...
$(function() {
	 ...
	 * @returns {Object}
	 */
	s2Marca.init = function (el) {
		// En caso no se envie el elemento, se toma uno por defecto
		el = (typeof el === 'undefined') ? $(s2Marca.elDefault) : el;

		this.select2 = el.select2({
			ajax: {
				url: BASE_URL + 'ar/ope/cmarcaxcliente/autocompletado',
				method: 'POST',
				dataType: 'json',
				delay: 250,
			...
			templateSelection: function (res) {
				if (res.id !== "") {
					...
					if (s2Marca.selected.id == 0) {
						// Solo cuando aun no es selccionado se marca.
						if (s2Marca.validate == 0) {
							if (typeof __elegirMarcaXCliente == "function") {
								__elegirMarcaXCliente(res);
							}
						}
					}
