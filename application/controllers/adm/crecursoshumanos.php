...
class Crecursoshumanos extends CI_Controller {
   ...
   /** CONTROL PERMISOS - VACACIONES - EXTRAS **/ 
    ...
    public function getempleados() { // Recupera empleado CBO	   
		...
		$resultado = $this->mrecursoshumanos->getempleados($ccia,$carea);
	...
	public function getlistempleadosperm() { // Recupera listado empleados
		...
		$resultado = $this->mrecursoshumanos->getlistempleadosperm($parametros);		
		echo json_encode($resultado);
	}
	...
	public function excelresumenperm($id_empleado) { // Recupera resumen de permisos por empleados en excel	
	 ...
	 /*Data*/
		...
		$rptCabecera = $this->mrecursoshumanos->getexcelresumenperm_cabecera($parametros);
		...
		$iv = 12;
		$rptVaca = $this->mrecursoshumanos->getexcelresumenperm_listvaca($parametros);
		if ($rptVaca){
		...
		$ix = 12;
		$rptExt = $this->mrecursoshumanos->getexcelresumenperm_listext($parametros);
		if ($rptExt){
		...
		$ip = 12;
		$rptPerm = $this->mrecursoshumanos->getexcelresumenperm_listperm($parametros);
		if ($rptPerm){
	...
	public function getlisthorasextras() { // Lista horas extras x empleado	   
		...
		$resultado = $this->mrecursoshumanos->getlisthorasextras($parametros);
		echo json_encode($resultado);
	}		
	public function getlistpermisos() { // Lista permisos x empleado	   
		...
		$resultado = $this->mrecursoshumanos->getlistpermisos($parametros);
		echo json_encode($resultado);
	}	
	...
	public function guardarpermiso() {	// Registrar Permisos		
		...
		$respuesta = $this->mrecursoshumanos->guardarpermiso($parametros);
	...
	public function guardarvacaciones() {	// Registrar Vacaciones		
		...
		$respuesta = $this->mrecursoshumanos->guardarvacaciones($parametros);
		echo json_encode($respuesta);
	}	
	public function sendEmailValidar() {  // Envio de Email para visto bueno  
		...
		$datopermisosvacaciones = $this->mrecursoshumanos->getpermisosvacaciones($idpermisosvacaciones);
