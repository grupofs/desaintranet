...
const s2ContacTottus = {
     ...
     * @var string
     */
    elDefault: 'select#area_id[role=menuitem]'
};
...
$(function() {
     ...
     * @returns {Object}
     */
    s2ContacTottus.init = function (el) {
        // En caso no se envie el elemento, se toma uno por defecto
        el = (typeof el === 'undefined') ? $(s2ContacTottus.elDefault) : el;

        this.select2 = el.select2({
            ajax: {
                url: BASE_URL + 'ar/evalprod/carea/autocompletado_contacto',
                method: 'POST',
                dataType: 'json',
                delay: 250,
            ...
            templateResult: function (res) {
                ...
                s2ContacTottus.selected.id = 0;
                s2ContacTottus.validate = 0;
                return res.text;
            },
            templateSelection: function (res) {
                if (res.id !== "") {
                    // Solo se ejecuta cuando se realiza una busqueda
                    if (s2ContacTottus.selected.id == 0) {
                        // Solo cuando aun no es selccionado se marca.
                        if (s2ContacTottus.validate == 0) {
                            if (typeof __elegirContacTottus == "function") {
                                __elegirContacTottus(res);
                            }
                        }
                    }
                    // Se cambia el estado para no voler a seleccionar
                    s2ContacTottus.validate = 1;
                    return res.text;
                } else {
                    return 'Elegir';
                }
