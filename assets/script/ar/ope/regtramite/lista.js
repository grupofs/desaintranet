			...
			result.forEach(function(item, pos) {
				...
				if (item.data && Array.isArray(item.data)) {
					...
					item.data.forEach(function(tramite) {
						...
						rows += '<button type="button" role="button" class="btn btn-outline-secondary btn-sm rounded-pill" >';
						rows += '<i class="fa fa-eye" ></i> Ver';
						rows += '</button>';
						rows += '<button type="button" role="button" class="btn btn-outline-danger btn-sm rounded-pill ml-1" >';
						rows += '<i class="fa fa-edit" ></i> Editar';
						rows += '</button>';
						rows += '</div>';
						rows += '</td>';
