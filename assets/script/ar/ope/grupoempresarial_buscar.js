...
$(function() {
	 ...
	 * @returns {Object}
	 */
	s2GrupoEmpresarial.init = function (el) {
		// En caso no se envie el elemento, se toma uno por defecto
		el = (typeof el === 'undefined') ? $(s2GrupoEmpresarial.elDefault) : el;

		this.select2 = el.select2({
			ajax: {
				url: BASE_URL + 'ar/ope/cgrupoempresarial/autocompletado',
				method: 'POST',
				dataType: 'json',
				delay: 250,
				data: function (params) {
					return {
						...
						params: s2GrupoEmpresarial.params
					};
				},
			...
			templateResult: function (res) {
				...
				s2GrupoEmpresarial.selected.id = 0;
				s2GrupoEmpresarial.validate = 0;
				return res.text;
			},
			templateSelection: function (res) {
				if (res.id !== "") {
					// Solo se ejecuta cuando se realiza una busqueda
					if (s2GrupoEmpresarial.selected.id == 0) {
						// Solo cuando aun no es selccionado se marca.
						if (s2GrupoEmpresarial.validate == 0) {
							if (typeof __elegirGrupoEmpresarial == "function") {
								__elegirGrupoEmpresarial(res);
							}
						}
					}
					// Se cambia el estado para no voler a seleccionar
					s2GrupoEmpresarial.validate = 1;
					return res.text;
				} else {
					return 'Elegir';
				}
